...
import { CourseMock } from "@/lib/mock-data";
...
export default function CoursesPageClient({ initialCourses }: CoursesPageClientProps) {
  const [courses] = useState<CourseMock[]>(initialCourses);
    ...
    <div className="space-y-8">
      ...
      {courses.length === 0 ? (
        <div className="glass-card rounded-2xl p-12 border border-zinc-800 text-center space-y-4">
          ...
          <h3 className="text-zinc-300 font-bold">No hay cursos disponibles</h3>
          <p className="text-zinc-500 text-xs">
            Pronto añadiremos nuevo contenido. ¡Vuelve más tarde!
          </p>
        </div>
      ) : (
